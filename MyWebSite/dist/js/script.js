...
document.addEventListener("DOMContentLoaded", function() {
    const btn = document.getElementById("skills_scroll_link");
    if (btn) {
        btn.addEventListener("click", function() {
            scrollToSection("skills_id");
        });
    }
});
...
document.getElementById("contacts_scroll_link").addEventListener("click", function() {
    scrollToSection("myForm");
});
document.getElementById("link_scroll_about").addEventListener("click", function() {
    scrollToSection("aboutMe");
});
...
(function ($) {
    $(function() {
        ...
        $('[data-modal=modal]').on('click', function() {
            ...
            if (!isFormValid($form)) {
                alert('Пожалуйста заполните обязательные поля перед оптравкой зaявки, Имя, Email, Согласие с политикой');
                return;
            }
